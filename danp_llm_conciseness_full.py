...
Usage:
  python danp_llm_conciseness_full.py --n_train 32 --n_eval 50 --epochs 5 --batch_size 2
  python danp_llm_conciseness_full.py --model_name Qwen/Qwen2.5-0.5B-Instruct --output_dir ./out_danp_full


   # Minimal run – 4 train, 4 eval, 1 epoch, batch 2
    python danp_llm_conciseness_full.py --n_train 4 --n_eval 4 --epochs 1 --batch_size 2

    # With verbose logging (gradient/update stats)
    python3 danp_llm_conciseness_full.py --n_train 4 --n_eval 4 --epochs 4 --batch_size 2 --verbose

    python3 danp_llm_conciseness_full.py --n_train 4 --n_eval 4 --epochs 4 --batch_size 2 --verbose --eta 1e-4

    # Population size 4 (more stable gradient estimate)
    python danp_llm_conciseness_full.py --n_train 8 --n_eval 4 --epochs 2 --batch_size 2 --n_population 4

    # Shorter sequences
    python danp_llm_conciseness_full.py --n_train 4 --n_eval 4 --epochs 1 --batch_size 2 --max_length 256
"""
...
parser.add_argument('--np_include', type=str, default='all', choices=['head', 'attn', 'mlp', 'all'],
                    help='Layers to apply DANP updates: all (default), attn, mlp, head')
...
parser.add_argument(
    ...
    default='ce',
    choices=['ce', 'reward'],
    help='ce: teacher-forcing cross-entropy for δL; reward: generate + reward(generated, target), δL=R_clean-R_noisy',
)
parser.add_argument('--max_new_tokens', type=int, default=128, help='Used when --objective reward (generation)')
parser.add_argument('--reward_do_sample', action='store_true', help='Sample during generation (reward mode)')
...
def _infer_total_blocks(model) -> int:
    # LLaMA/Qwen: model.model.layers.N or model.transformer.h.N
    names = [n for n, _ in model.named_modules() if re.search(r'\.layers\.(\d+)\.', n)]
    if not names:
        # GPT-2: model.transformer.h.N
        names = [n for n, _ in model.named_modules() if re.search(r'\.h\.(\d+)\.', n)]
    if not names:
        return 0
    pat = r'\.layers\.(\d+)\.' if 'layers' in names[0] else r'\.h\.(\d+)\.'
    idxs = [int(re.search(pat, n).group(1)) for n in names if re.search(pat, n)]
    return max(idxs) + 1 if idxs else 0


def _block_index(name: str) -> int | None:
    """Extract block index from layer name (layers.N or h.N)."""
...
def _is_target_linear(include: str, name: str, mod, last_k: int, total_blocks: int) -> bool:
    ...
    if include == 'attn':
        return any(k in lname for k in ('attn', 'attention', 'q_proj', 'k_proj', 'v_proj', 'o_proj', 'in_proj', 'c_attn', 'c_proj')) and 'mlp' not in lname
    if include == 'mlp':
        return any(k in lname for k in ('mlp', 'ffn', 'gate_proj', 'up_proj', 'down_proj', 'fc1', 'fc2', 'c_fc')) or ('c_proj' in lname and 'mlp' in lname)
    return True
...
def _get_R_in_producer(name, in_features, targets_by_name):
    ...
    GPT-2: c_attn <- prev block mlp.c_proj; attn.c_proj <- (attention output, no linear); mlp.c_fc <- attn.c_proj; mlp.c_proj <- c_fc.
    """
    if "down_proj" in name:
        ...
        if producer in targets_by_name and targets_by_name[producer].out_features == in_features:
            return producer
    # GPT-2: h.N.attn.c_attn, h.N.attn.c_proj, h.N.mlp.c_fc, h.N.mlp.c_proj
    elif "mlp.c_proj" in name:
        producer = name.replace("mlp.c_proj", "mlp.c_fc")
        if producer in targets_by_name and targets_by_name[producer].out_features == in_features:
            return producer
    elif "mlp.c_fc" in name:
        producer = name.replace("mlp.c_fc", "attn.c_proj")
        if producer in targets_by_name and targets_by_name[producer].out_features == in_features:
            return producer
    elif "attn.c_attn" in name:
        # Input from previous block's mlp.c_proj (GPT-2) or self_attn.o_proj (LLaMA)
        m = re.search(r'\.h\.(\d+)\.', name)
        if m:
            idx = int(m.group(1))
            if idx > 0:
                producer = name.replace(f"h.{idx}.attn.c_attn", f"h.{idx-1}.mlp.c_proj")
                if producer in targets_by_name and targets_by_name[producer].out_features == in_features:
                    return producer
    return None
...
def compute_loss(model, tokenizer, batch, device, max_length, label_ignore_id=-100):
    """Teacher-forcing cross-entropy loss on target tokens only."""
    input_ids, attention_mask, labels = _prepare_batch(
        batch, tokenizer, device, max_length, label_ignore_id
    )
...
def evaluate_mean_reward(
    model, tokenizer, pairs, device, max_new_tokens, do_sample, compute_reward_fn=None,
):
    """Mean reward on (prompt, target) pairs via generation without DANP hooks."""
    if compute_reward_fn is None:
        compute_reward_fn = default_compute_reward
    model.eval()
    s = 0.0
    with torch.no_grad():
        for prompt, target in pairs:
            ...
            s += float(compute_reward_fn(text, target))
    return s / max(1, len(pairs))


def default_compute_reward(generated_text: str, target_text: str) -> float:
    """Higher is better; same formula as conciseness_task/wp_conciseness_task.py (wp_conciseness)."""
    if target_text is None:
        return 0.0
    return -abs(len(generated_text) - len(target_text))


def _forward_reward_generate(
    model, tokenizer, prompt, target, device, max_new_tokens, do_sample,
    hook: DANPFullHook, add_noise: bool, compute_reward_fn,
):
    """
    One generation pass with hooks; returns scalar reward and captured activations.
    Activations are from the last forward step of generate() per layer.
    """
    hook.attach(add_noise=add_noise, capture=True)
    with torch.no_grad():
        inp = tokenizer(prompt, return_tensors="pt", padding=True, padding_side="left")
        input_ids = inp["input_ids"].to(device)
        attention_mask = inp["attention_mask"].to(device)
    ...
    captured = dict(hook._captured)
    hook.detach()
    text = tokenizer.decode(out[0], skip_special_tokens=True)
    r = float(compute_reward_fn(text, target))
    return r, captured
...
def compute_danp_grad_estimate_per_sample(
    ...
    objective='ce': δL = L_noisy - L_clean (cross-entropy, teacher forcing).
    objective='reward': δL = R_clean - R_noisy (higher reward is better); uses generate().
    """
    if compute_reward_fn is None:
        compute_reward_fn = default_compute_reward
    ...
    if objective == "reward":
        prompt, target = single_example
        R_clean, cap_raw = _forward_reward_generate(
            model, tokenizer, prompt, target, device, max_new_tokens, reward_do_sample,
            hook, add_noise=False, compute_reward_fn=compute_reward_fn,
        )
        captured_clean = {
            k: (v[0].clone(), v[1].clone(), None)
            for k, v in cap_raw.items()
        }
        L_clean = R_clean
    else:
        batch = [single_example]
        input_ids, attention_mask, labels = _prepare_batch(
            batch, tokenizer, device, max_length, label_ignore_id=-100
        )
    ...
    for pop_idx in range(n_population):
        if objective == "reward":
            prompt, target = single_example
            R_noisy, cap_noisy_raw = _forward_reward_generate(
                model, tokenizer, prompt, target, device, max_new_tokens, reward_do_sample,
                hook, add_noise=True, compute_reward_fn=compute_reward_fn,
            )
            captured_noisy = {}
            for k, v in cap_noisy_raw.items():
                captured_noisy[k] = (v[0].clone(), v[1].clone(), v[2].clone() if v[2] is not None else None)
            delta_L_sum += R_clean - R_noisy
            L_noisy = R_noisy
        else:
            batch = [single_example]
            input_ids, attention_mask, labels = _prepare_batch(
        ...
        norm_sq = (delta_a_concat ** 2).sum().item() + 1e-6
        N = delta_a_concat.numel()

        if objective == "reward":
            delta_L = R_clean - R_noisy
        else:
            delta_L = L_noisy - L_clean
        if not np.isfinite(L_clean) or not np.isfinite(L_noisy):
            continue  # Skip this sample if loss is NaN/Inf
        delta_L_clamped = np.clip(delta_L, -1e4, 1e4)
    ...
    for name, _ in hook._targets:
        ...
        dec_update = torch.clamp(dec_update, -0.1, 0.1)  # Prevent R from exploding
        if torch.isnan(dec_update).any() or torch.isinf(dec_update).any():
            continue
...
def run_danp_batch_step(
    model, tokenizer, batch, device, hook, eta, alpha, max_length,
    objective="ce",
    compute_reward_fn=None,
    max_new_tokens=128,
    reward_do_sample=False,
    ...
    for i in range(batch_size):
        weight_updates, decorrelation_updates, L_clean, delta_L = compute_danp_grad_estimate_per_sample(
            model, tokenizer, batch[i], device, hook, eta, alpha, max_length,
            objective=objective,
    ...
    # Average and apply weight updates (skip NaN/Inf)
    with torch.no_grad():
        for key, acc in accumulated_weight_updates.items():
            upd = acc / batch_size
            if torch.isnan(upd).any() or torch.isinf(upd).any():
                continue
            if key.endswith(".weight"):
                mod_path = key[:-7]
    ...
    # Apply averaged decorrelation updates (skip NaN/Inf)
...
def main(args=None):
    ...
    if args.objective == "reward":
        baseline_eval = evaluate_mean_reward(
            ...
            args.max_new_tokens, args.reward_do_sample, default_compute_reward,
        )
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(eval_data), args.batch_size):
                ...
                loss = compute_loss(model, tokenizer, batch, device, args.max_length).item()
    ...
    for epoch in range(args.epochs):
        ...
        for step, start in enumerate(pbar):
            ...
            pbar.set_postfix({"metric": f"{L_clean:.4f}", "delta": f"{delta_L:.4f}"})
        ...
        if (epoch + 1) % args.eval_interval == 0:
            ...
            if args.objective == "reward":
                ...
                with torch.no_grad():
                    ...
                    for i in range(0, len(eval_data), args.batch_size):
                        batch = eval_data[i:i + args.batch_size]
                        evals.append(compute_loss(model, tokenizer, batch, device, args.max_length).item())
                eval_loss = np.mean(evals)
            eval_losses_hist.append(eval_loss)
            tag = "reward" if args.objective == "reward" else "loss"
            print(f"[Epoch {epoch+1}] Train: {train_loss:.4f}, Eval {tag}: {eval_loss:.4f}")

    print(f"\nTraining complete. Final train metric: {train_losses[-1]:.4f}")
    if eval_losses_hist:
        print(f"Final eval: {eval_losses_hist[-1]:.4f}")

    # Save results for plotting
    n_eval = len(eval_losses_hist)
    eval_epochs = list(range(1, n_eval + 1))
    results = {
        "config": {
